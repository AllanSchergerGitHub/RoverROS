...
class RoverDriveKeyOp():
    def __init__(self, args):
        ...
        if len(args) > 2:
            ...
            cmd_topic = rospy.get_namespace() + 'ackermann_cmd'
        ...
        for key in key_bindings:
            key_bindings[key] = (key_bindings[key][0] * float(max_speed)/5,
                ...
                if key in key_bindings.keys():
                    if key == control_keys['space']:
                        ...
                        self.steering_angle = clip(
                            self.steering_angle, self.steering_angle_range[0],
    ...
    def exit(self):
        rospy.loginfo('Halting motors, alignining wheels and exiting...')
        self.settings = termios.tcgetattr(sys.stdin)
        ackermann_cmd_msg = AckermannDriveStamped()
...
if __name__ == '__main__':
    rospy.init_node('rover_keyop')
    keyop = RoverDriveKeyOp(sys.argv[1:len(sys.argv)])
